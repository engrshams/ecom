            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <TableComponent :data="categories" :columns="columns" :page-size="5">
                    ...
                    <template #actions-slot="{ row }">
                        <div class="space-x-2">
                            <Link :href="`categories/${row.id}/edit`" class="bg-yellow-500 text-white px-2 py-1 rounded-md">Edit</Link>
                            <button @click="deleteCategory(row.id)" class="bg-red-500 text-white px-2 py-1 rounded-md">Delete</button>
                        </div>
                    </template>
                </TableComponent>
            </div>
